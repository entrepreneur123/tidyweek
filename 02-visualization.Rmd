...
# color aesthetic
ggplot(data = bechdel) +
    geom_point(mapping = aes(x = budget , y =domgross , color =binary ))
...
# alpha aesthetic 
ggplot( data = bechdel) +
    geom_point(mapping = aes(x =budget , y =domgross,shape=binary,color=intgross),alpha=0.4)
```
...
ggplot(bechdel, aes(x = binary ,
                  y = budget)) +
  geom_col(aes(fill = budget), width =0.2 )
```
...
# add color scale manually
  scale_fill_manual(values = c("red", "#F6AE2D")) +
  theme_void()
```
...
bechdel %>% count(clean_test) %>% ggplot() +
    ...
    geom_text(aes(x= clean_test, y = n, label = n),vjust=-0.5) +
...
bechdel %>% count(clean_test) %>% ggplot() +
    geom_col(aes(x=reorder(clean_test,n),y=n,fill= clean_test)) +
    geom_label(aes(x= clean_test, y = n, label = n)) +coord_flip()
    labs(title = "Distribution of test", y = "Count",x = "")
```
```{r}
